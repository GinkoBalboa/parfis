...
namespace parfis
{
    struct Config 
    {        
        static int getDomainId(const std::string& cstr);
        static std::string getDomainName(uint32_t id);

        static std::map<std::string, uint32_t> s_domainNameIdMap;

        static std::map<uint32_t, std::map<std::string, uint32_t>> s_paramNameIdMap;
    };
